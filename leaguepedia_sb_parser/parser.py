...
class Parser(object):
    ...
    GAME_TEXT = '{{{{Scoreboard/Season 16{}\n{}\n}}}}'
    ...
    def get_final_team_name(self, team_name, team_key):
        ...
        if result is None:
            ...
            self.warnings.append(
                'Final team name for {} is missing (original: `{}`) - maybe you have to set |short= in the \
                 Participants section if the code differs from what we use in Teamnames. You may also need to \
                 RefreshTeamnames or blank edit the team page. Inferred event: {}'.format(
    ...
    def get_duration(duration):
        ...
        # wouldn't work if we used the time lib here for games longer than an hour, we need to do it manually
        return '{}:{}'.format(str(math.floor(duration / 60)), str(math.floor(duration % 60)).zfill(2))
